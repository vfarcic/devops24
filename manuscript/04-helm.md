...
It is important to stress that publishing ports (beyond 80 and 443) is not a good idea. They are harder to manage, and they are not very user-friendly. We cannot expect users of our applications to remember the ports. Instead, we use domains to distinguish one application from another. Unlike merely opening ports, domains also allow us better control of the traffic and are easier to tie with certificates. But, you probably know all that. You are almost certainly already having your applications behind one or more domains. So, there's no need for me convincing you that domains are a good thing. Also, you already know how to leverage Ingress to forward traffic from a domain to an application. What you might not know is how to create valid domains without registering them through a registrar.
