...
When we (humans) try to access a Kubernetes cluster with RBAC enabled, we are authenticated as users. Our username provides an identity that the API server uses to decide whether we are allowed to perform intended actions. Similarly, processes running inside containers might also need to access the API. In such cases, they are authenticated as a specific ServiceAccount.
...
* [kops.sh](https://gist.github.com/2a3e4ee9cb86d4a5a65cd3e4397f48fd): **kops in AWS** with 3 t2.small masters and 2 t2.medium nodes spread across three availability zones, and with **nginx Ingress** (assumes that the prerequisites are set through [Appendix B](#appendix-b)).
...
The `default` ServiceAccount is created for every Namespace in the cluster. It is, in a way, similar to the default StorageClass. If a Pod does not have a ServiceAccount, it'll get the `default`. That ServiceAccount has insufficient privileges, and it cannot do almost anything. We can bind a more permissive role to the `default` ServiceAccount, but that would be a very uncommon solution to the problem.

We need to associate the Jenkins process with an entity that has more permissions. As a minimum, we should be able to list the Pods in the `jenkins` Namespace. To do that, we have to learn about ServiceAccounts first.

We'll delete the `jenkins` Namespace and search for a simple way to explore ServiceAccounts.
...
The Dockerfile is so uneventful and straightforward that there's probably no need going through it. It's a `kubectl` binary in an `alpine` based image. No more, no less.
...
At the moment, there is only one ServiceAccount called `default`. We already saw the limitations of that account. It is stripped from (almost) all privileges. If we check the other Namespaces, we'll notice that all of them have only the `default` ServiceAccount. Whenever we create a new Namespace, Kubernetes creates that account for us.
...
Just as with RBAC users and groups, RoleBindings are tying Roles to ServiceAccounts. Since our objective to provide read-only permissions that can be fulfilled with the ClusterRole `view`, we did not need to create a new Role. Instead, we're binding the ClusterRole `view` with the ServiceAccount of the same name.

If you are already experienced with RBAC applied to users and groups, you probably noticed that ServiceAccounts follow the same pattern. The only substantial difference, from a YAML perspective, is that the `kind` of the subject is now `ServiceAccount`, instead of being `User` or `Group`.
...
So far, we did not explore the effect of ServiceAccounts on Namespaces other than those where we created them, so we'll create another one called `test2`.
...
The two Roles could have been created with different permissions. They are the same (for now) only for simplicity reasons. We could have simplified the definition by defining a single ClusterRole and saved ourselves from having two Roles (one in each Namespace). However, once we get back to Jenkins, we'll probably want to have different permissions in different Namespaces, so we're defining two Roles as a practice.
