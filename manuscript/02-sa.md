...
Now we need to go through the setup wizard. It's a boring process that we already did before and I'm sure you're not thrilled with the prospect of going through it. However, we're still missing knowledge and tools that will allow us to automate the process, so we'll have to do the boring part manually.
...
Please copy the output and paste it into the *Administrator password* field. Click the *Continue* button, followed with a click to *Install suggested plugins* button. Fill in the *Create First Admin User* fields and press the *Save and Finish* button. Finally, Jenkins is ready and only a click away. Please press the *Start using Jenkins* button.
...
The plugin adds Kubernetes as yet another *Cloud* provider. Please expand the *Add a new cloud* drop-down list in the *Cloud* section, and select *Kubernetes*. The cloud section should be somewhere close to the bottom of the screen.
...
A `default` ServiceAccount is created for every Namespace in the cluster. It is, in a way, similar to the default StorageClass. If a Pod does not have a ServiceAccount, it'll get the `default`. That ServiceAccount has very limited privileges and it cannot do almost anything. We can bind more permissive role to the `default` ServiceAccount but that would be a very uncommon solution to the problem.

We need to associate Jenkins process with an entity that has more permissions. As a minimum, we need to be able to list the Pods in the `jenkins` Namespace. To do that, we need to learn about ServiceAccounts first.

We'll delete `jenkins` Namespace and explore ServiceAccounts in our quest to enable and control the communication between the API server and processes running in containers.
...
Since we did not specify any ServiceAccount, the `default` one that was created with the Namaspace was assigned to the Pod automatically.

We might be able to dig a bit more information from within the `kubect` container.
...
We can see that it created three soft-links (`ca.crt`, `namespace`, and `token`) that point to a directory that contains the actual files. You should be able to guess what those files contain. The token and the certificate are used for authentication when communicating with the API server. The third file contains only the Namespace in which the Pod is running.
...
We saw the limitations behind the `default` account. We'll try to overcome them by creating our own ServiceAccounts.
...
At the moment, there is only one ServiceAccount called `default`. We already saw that account cannot do much. It is stripped from (almost) all the privileges. If we check the other Namespaces, we'll notice that all of them have only the `default` ServiceAccount. Whenever we create a new Namespace, Kubernetes creates that account for us.
...
Just as with RBAC users and groups, RoleBindings are tying Roles or ClusterRoles with ServiceAccounts. Since our objective to provide read-only permissions can be fulfilled with the ClusterRole `view`, we did not need to create a role ourselves. Instead, we're binding the ClusterRole `view` with the ServiceAccount with the same name.

If you are already experienced with RBAC applied to users and groups, you probably noticed that ServiceAccounts follow the same pattern. The only substantial difference, from YAML perspective, is that the `kind` of the subject is now `ServiceAccount`, instead being `User` or `Group`.
...
The output should show that a ServiceAccount and a RoleBinding named `view` were created.

Next, we'll list the ServiceAccount in the `default` Namespace and confirm that the new one was created.
...
We can see that the `ClusterRole` named `view` has a single subject with the ServiceAccount with the same name.

Now that we have a ServiceAccount that has enough permissions to view (almost) all the resources, we'll create a Pod with `kubectl` which we can use to explore permissions.
...
Let's see whether this time we can retrieve the Pods from the same Namespace.
...
W> ## A note to minishift users
W>
W> You should see three Pods instead of one. When we created an OpenShift cluster with minikube, it deployed Docker Registry and a Router to the `default` Namespace.

Even though you probably know the answer, we'll confirm that we can indeed only view the resources by trying to create a new Pod.
...
As expected, we cannot create Pods. We bound the ClusterRole `view` to the ServiceAccount and that allows us only read-only access to resources.
...
The Jenkins Kubernetes plugin needs to have full permissions on Pods. It should be able to create them, to retrieve logs from them, to execute processes, to delete them, and so on. The resources defined in `sa/pods.yml` should allow us just that.
...
So far, we did not explore the effect of ServiceAccount to Namespaces other than those where we created them, so we'll create one called `test2`.

```bash
kubectl create ns test2
```

Finally, we'll run `kubectl` as a Pod so that we can test the new account.
...
The only differences from the previous `kubectl` definition is in the `namespace` and the `serviceAccountName`, so there's probably no need for an explanation. Instead, we'll just proceed and `apply` the definition.
...
We already experienced the same result as with the `view` role, so let's try something different and try to create a Pod.
...
As you hopefully know, if we execute a `run` command without the `--restart=Never` argument, Kubernetes creates a Deployment, instead of a Pod.
...
We saw that, with the previous definition, we obtained permissions only within the same Namespace as the Pod attached to the ServiceAccount. In some cases that is not enough. Let's go back to Jenkins use-case. We might decide to run Jenkins master in one Namespace but run the builds in another. Or, we might create a pipeline that deploy a beta version of our application and tests it in one Namespace and, later on, deploy it to production in another. Surely there is a way to accommodate such needs.
...
We start by creating a ServiceAccount and a Role called `pods-all` in the `test1` Namespace. Further on, we're creating the same role but in the `test2` Namespace. Finally, we're creating RoleBindings in both Namespaces. The only difference between the two is in a single line. The RoleBinding in the `test2` Namespace has the `subjects` entry `namespace: test1`. With it, we are linking a binding from one Namespace to a ServiceAccount in another. As a result, we should be able to create a Pod in the `test1` Namespace that will have full permissions with Pods in both Namespaces.

The two RoleBindings could have been different. They are the same (for now) only for simplicity reasons. Also, we could have defined a ClusterRole and save ourselves from having two Roles (one in each Namespace). However, once we get back to Jenkins, we'll probably want to have different permissions in different Namespaces so we're defining two Roles.
...
The output shows that `no resources` were `found`. If we do not have permissions to view the files we'd get the already familiar `forbidden` message instead. To be on the safe side, we'll try to create a Pod in the `test2` Namespace.
...
The Pod was obviously created in the `test2` Namespace. Mission was accomplished and we can get out of the container and delete the Namespaces we created before we try to apply the knowledge about ServiceAccounts to Jenkins.
...
Now that we got a grip on ServiceAccounts, it should be relatively straight forward to correct the problem we experienced with Jenkins. As a reminder, we could not configure the Kubernetes plugin. We experienced the same `forbidden` message as when we tried to use `kubectl` container with the `default` ServiceAccount. Since now we know that ServiceAccounts provide permissions to processes running inside containers, all we have to do is to define one for Jenkins.

We'll move to a more complicated example. We'll try to run Jenkins master in one Namespace and run builds in another. That way we can have a clear separation between Jenkins and "random" stuff our builds might be doing. Through such separation we can guarantee that Jenkins will (probably) not be affected if we do something wrong in our builds.

Our example will not set up LimitRanges and ResourceQuotas. I'll assume that you're familiar with them and that you understand that any more serious setting must have them defined. If you're newbie to those things, please consult the official documentation or explore related chapters in [The DevOps 2.3 Toolkit: Kubernetes](https://amzn.to/2GvzDjy).
...
This time we are creating a second Namespace called `build`. Then, we're creating ServiceAccount `jenkins` in the namespace with the same name. Further on, we created a Role and a RoleBinding that provide required permissions. Both are in the `build` Namespace but they are associated with the ServiceAccount in the `jenkins` Namespace. As a result, our Jenkins should be able to create Pods in `build` but it won't be able to do anything in its own Namespace `jenkins`. That way we can be relatively safe that a problem in our builds will not affect Jenkins. If we specified ResourceQoutas and LimitRanges for both Namespaces, our solution would be even more bullet proof. But, since I assume that you know how to do that, I excluded them in an attempt to simplify the definition.
...
We can see from the output that the resources we created and all that's left is to wait until Jenkins rolls out.
...
Now that Jenkins is up-and-running, we should open it in browser and repeat the same setup steps we did before.
...
Please copy the output and paste it into *Administrator password* field. Click *Continue*, followed by the *Install suggested plugins* button. The rest of the setup requires you to *Create First Admin User*, so please go ahead. You don't need my help on that one.
...
Now that we have a ServiceAccount that grants us the required permissions, we can simply click the *Test Connection* button and confirm that it works.
...
Did we do something wrong? We didn't. That is the desired behavior. By not specifying a Namespace, Jenkins checked whether it has necessary permission in the Namespace it runs on. If we try to invoke Kube API from a container, it'll always use the same Namespaces as the one the container is in. Jenkins is no exception. On the other hand, our YAML explicitly defined that we want to create Pods in the `build` Namespace. Let's fix that.

Please type *build* in the *Kubernetes Namespace* field and click the *Test Connection* button again.

This time, the output shows that the `connection test` was `successful`. We successfully configured Jenkins' Kubernetes plugin to operate inside the `build` Namespace. Still, there is one more thing missing.

When we create a job that uses Kubernetes Pods, an additional container will be added. That container will use JNLP to establish communication with the master Jenkins. We need to specify a valid address JNLP can use to connect to the master. Since the Pods will be in the `build` Namespace and the master is in `jenkins`, we need to use the longer DNS name that specifies both the name of the service (`jenkins`) as well as the Namespace (also `jenkins`). On top of all that, our master is configured to respond use `/jenkins` as the root path. All the all, the full address Pods can use to communicate with Jenkins master is should be `http://[SERVICE_NAME].[NAMESPACE]/[PATH]`. Since all those components are `jenkins`, the "real" address is `http://jenkins.jenkins/jenkins`. Please type it to the *Jenkins URL* field and click the *Save* button.

Now we're ready to create a job that'll test that everything works as expected.

Please click the *New Item* link from the left-hand menu to open a screen for creating jobs. Type *my-k8s-job* in the *item name* field, select *Pipeline* as the type, and click the *OK* button. Once inside the job configuration screen, click the *Pipeline* tab and write the script that follows in the *Pipeline Script* field.
...
I> If you prefer to copy and paste, the job is available in the [my-k8s-job Gist](https://gist.github.com/2cf872c3a9acac51409fbd5a2789cb02).

The job is fairly simple. It uses `podTemplate` to define a `node` that will contain two containers. One of those is `golang` and the other is `maven`. In both cases the `command` is `cat`. Without a long-running command (process `1`), the container would exit immediately, Kubernetes would detect that and start another container based on the same image, it would fail again, and so on. Without the main process running, we'd enter into a never-ending loop.
...
Now that we have a job, we should run it and thus validate that the integration with Kubernetes indeed works.

Please click the *Open Blue Ocean* link from the left-hand menu followed with the *Run* button.
...
You might be wondering why are there three containers even though we specified two. Jenkins added the third container to the Pod definition. It contains JNLP that is in charge of communication between Pods acting as nodes and Jenkins masters. From user's perspective, JNLP is non-existent. It is a transparent process we do not need to worry about.
...
Once Jenkins finished executing the instructions from the job, it issued a command to Kube API to terminate the Pod. There's no need to waste resources when we're not doing anything.

We're done with Jenkins, so let's remove both Namespaces before we move into the next use-case.
...
We still have one more pending issue that we can solve with ServiceAccounts. In the previous chapter we tried to use `cvallance/mongo-k8s-sidecar` container in hopes it'll dynamically create a MongoDB replica set. We failed because, at that time, we did not know how to create sufficient permissions that would allow the side-car to do its job. Now we know better.

Let's take a look at an update version of our *go-demo-3* application.s
...
Just as with Jenkins, we are a ServiceAccount, a Role, and a RoleBinding. Since the side-car needs only to list the Pods, the Role is this time more restrictive than the one we created for Jenkins. Further down, in the StatefulSet, we added the `serviceAccountName`. None of those things are new. We're applying the same logic to the side-car as to Jenkins.

Since there's no need to a lengthy discussion, we'll just move on and `apply` the definition.
...
If you waited for a while, the output should be as follows.
...
ServiceAccounts combined with Roles and RoleBindings are an essential component for continuous deployment or any other processes that need to communicate with Kubernetes. The alternative is to run an unsecured cluster which is not an option for any but smallest organizations. RBAC is required when there's more than one person operating or using a cluster. If RBAC is enabled, ServiceAccounts are a must. We'll use them heavily in the chapters that follow.
