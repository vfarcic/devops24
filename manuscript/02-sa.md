...
W> ## A note to **minishift** users
W>
W> Due to the changes required for OpenShift, we'll use a different YAML specification. Please execute `oc create -f sa/jenkins-oc.yml --record --save-config` instead of the command that follows.
